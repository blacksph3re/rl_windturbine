...
This equation uses the notion of a state-value function, also called Q function. In the Bellman Equation, it describes the total accumulated return until infinity taking an action $a$ in state $s$, receiving reward $r$, ending up in state $s'$ and then following the optimal policy afterwards. With the help of this function, we can compare different actions in our current state. And also, this small definition gives us a hint on how to create this function. We could initialize an array for all states and actions and then repeatedly loop through the array and fill it according to the above equation. However, this table is only computationally feasible when we have small finite state and action spaces. If any of the two spaces is big or even continuous, we have to replace the table by an estimator for Q.
